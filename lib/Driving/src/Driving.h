...
class Driving {
    ...
    private:
        ...
        //----State defaults----
        static constexpr uint32_t DEFAULT_MAX_DRIVE_TIME      = 50000;
        static constexpr uint32_t SLOW_MAX_DRIVE_TIME         = 50000;  // extended drive timeout while slow-speed is active (a slow tile takes longer than 5s)
        static constexpr uint16_t DEFAULT_MAX_ENCODER_TIME    = 50000;
        static constexpr uint16_t DEFAULT_MAX_TURN_TIME       = 50000;
        static constexpr uint16_t DEFAULT_RAMP_CHECK_DURATION = 50000;
        #ifdef _MSC_VER
            #pragma endregion
